...
import { account, databases, isAppwriteConfigured } from "@/lib/pocketbase";
    ...
    const checkSession = async () => {
        ...
        try {
            ...
            try {
                ...
                // 402 = Payment Required (Appwrite billing/quota exceeded)
                if (errorCode === 402) {
                    console.error("Appwrite quota exceeded (402). Check your Appwrite billing/plan.");
                    // Still allow the user in with a default role — don't lock them out
                    setRole("user");
                    return;
                }

                // Only attempt heal on genuine 404 (document not found)
                if (errorCode === 404) {
                    ...
                    // Unknown error — log it but don't destroy the session
                    console.error("Unexpected error fetching user document:", errorCode, error);
                    setRole("user");
                }
            }
